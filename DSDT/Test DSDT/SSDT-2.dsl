 ...
 *
 * Disassembly of SSDT-2.aml, Mon Jan 22 16:30:48 2018
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x0000431A (17178)
 *     Revision         0x02
 *     Checksum         0x83
 *     OEM ID           "DptfTa"
...
DefinitionBlock ("", "SSDT", 2, "DptfTa", "DptfTabl", 0x00001000)
{
    ...
    External (_SB_.SCWT, FieldUnitObj)    // (from opcode)
    External (_SB_.TJMX, FieldUnitObj)    // (from opcode)
    External (_TZ_.THRM, UnknownObj)    // (from opcode)
    External (APPE, FieldUnitObj)    // (from opcode)
    ...
    External (GN1E, FieldUnitObj)    // (from opcode)
    External (GN2E, FieldUnitObj)    // (from opcode)
    External (GN3E, FieldUnitObj)    // (from opcode)
    External (GN4E, FieldUnitObj)    // (from opcode)
    ...
    External (MEMS, FieldUnitObj)    // (from opcode)
    External (MPL0, FieldUnitObj)    // (from opcode)
    External (MPL1, FieldUnitObj)    // (from opcode)
    External (MPL2, FieldUnitObj)    // (from opcode)
    External (ODV0, FieldUnitObj)    // (from opcode)
    External (ODV1, FieldUnitObj)    // (from opcode)
    External (ODV2, FieldUnitObj)    // (from opcode)
    External (ODV3, FieldUnitObj)    // (from opcode)
    External (ODV4, FieldUnitObj)    // (from opcode)
    External (ODV5, FieldUnitObj)    // (from opcode)
    External (P8XH, MethodObj)    // 2 Arguments (from opcode)
    External (PDC0, IntObj)    // (from opcode)
    External (PERE, FieldUnitObj)    // (from opcode)
    External (PPSZ, FieldUnitObj)    // (from opcode)
    External (PTMC, FieldUnitObj)    // (from opcode)
    External (PTRA, FieldUnitObj)    // (from opcode)
    External (PWRE, FieldUnitObj)    // (from opcode)
    External (PWRP, FieldUnitObj)    // (from opcode)
    ...
    Scope (\_SB.PCI0.B0D4)
    {
        ...
        Method (CPL1, 0, NotSerialized)
        {
            Store (0x02, Index (\_SB.PCI0.B0D4.NPCC, Zero))
            Store (Zero, Index (DerefOf (Index (\_SB.PCI0.B0D4.NPCC, One)), Zero))
            Store (0x1F40, Index (DerefOf (Index (\_SB.PCI0.B0D4.NPCC, One)), One))
            Store (0x4E20, Index (DerefOf (Index (\_SB.PCI0.B0D4.NPCC, One)), 0x02))
            Store (0x03E8, Index (DerefOf (Index (\_SB.PCI0.B0D4.NPCC, One)), 0x03))
            Store (0x03E8, Index (DerefOf (Index (\_SB.PCI0.B0D4.NPCC, One)), 0x04))
            Store (0xFA, Index (DerefOf (Index (\_SB.PCI0.B0D4.NPCC, One)), 0x05))
            Store (One, Index (DerefOf (Index (\_SB.PCI0.B0D4.NPCC, 0x02)), Zero))
            Store (0x0DAC, Index (DerefOf (Index (\_SB.PCI0.B0D4.NPCC, 0x02)), One))
            Store (0x61A8, Index (DerefOf (Index (\_SB.PCI0.B0D4.NPCC, 0x02)), 0x02))
    ...
    Scope (\_SB.IETM)
    {
        ...
        Method (PSVT, 0, Serialized)
        {
            ...
            Name (PSV2, Package (0x01)
            {
                0x02
            })
            Name (PSV3, Package (0x01)
            {
                0x02
            })
            While (One)
            {
                ...
                If (LEqual (_T_0, 0x64))
                {
                    If (LEqual (ToInteger (\_SB.SCWT), 0x96))
                    {
                        Return (PSV1)
                    }
                }
